...
const fetchFakerData = async () => {
  let response = await(await fetch("https://randomuser.me/api/?results=30&nat=us&")).json()
  console.log(response)


    for (let people of response.results) {
      console.log(people);
      const pepopleContainer = document.createElement("div");
      pepopleContainer.className = "people";
      const nameOfPeople = document.createElement("h3");
      const img = document.createElement("img");
      img.height = "250";
      img.width = "250";
      img.src = people.picture.large;
      nameOfPeople.innerHTML = people.name.first + " " + people.name.last;
      pepopleContainer.append(nameOfPeople, img);
      mainContainer.append(pepopleContainer);
    }
